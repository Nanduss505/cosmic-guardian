...
function updateHUD() {
    ...
    let stageName = "Tahap 1";
    if (level === 1) stageName = "Tahap 1: Nebula (G1)";
    else if (level === 2) stageName = "Tahap 1: Nebula (G2)";
    else if (level === 3) stageName = "Tahap 2: Asteroid (G1)";
    else if (level === 4) stageName = "Tahap 2: Asteroid (G2)";
    else if (level === 5) stageName = "Tahap 3: Pusaran Boss";
...
document.getElementById("savePlayerBtn").onclick = () => {
    ...
    if (username === "") {
        alert("⚠️ Nama Pilot tidak boleh kosong!");
        return;
    }
    
    // 2. Validasi: Maksimum 12 karakter
    if (username.length > 12) {
        alert("⚠️ Nama Pilot tidak boleh lebih dari 12 karakter!");
        return;
    }
    
    // 3. Validasi: Tidak boleh sama/duplikat (diperiksa terhadap Papan Skor lokal)
    const leaderboard = JSON.parse(localStorage.getItem("galaxyLeaderboard")) || [];
    const isDuplicate = leaderboard.some(entry => entry.name.toLowerCase() === username.toLowerCase());
    if (isDuplicate) {
        alert("⚠️ Nama Pilot ini sudah terdaftar di Papan Peringkat! Silakan gunakan Call Sign lain.");
        return;
    }
