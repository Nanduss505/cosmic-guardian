...
function createExplosion(x, y, size = 80) {
    ...
    // Particle sparks
    if (typeof createParticle === "function") {
        const particleCount = Math.floor(15 * (size / 80));
        const offset = size / 2;
        for (let i = 0; i < particleCount; i++) {
            const speedFactor = size > 80 ? 3.0 : 1.8;
            createParticle(x + offset, y + offset, Math.random() < 0.5 ? "#ffaa00" : (Math.random() < 0.5 ? "#ff3300" : "#ffcc00"), speedFactor);
        }
    }
}
...
function drawEnemies() {
    ...
    if (stageIntroActive) {
        ...
        let titleText = `TAHAP 01: SIANTANARUS`;
        if (activeLevelStage === 2) {
            titleText = `TAHAP 02: MUKHSINIUS`;
        } else if (activeLevelStage === 3) {
            titleText = `TAHAP 03: RENGASINIUS`;
        }
        ctx.fillText(titleText, canvas.width / 2, canvas.height / 2 - 15);
